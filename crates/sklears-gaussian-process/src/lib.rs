...
//     ConstantNoise, GaussianProcessNoise, HeteroscedasticGaussianProcessRegressor,
//     LearnableNoiseFunction, LinearNoise, NeuralNetworkNoise, NoiseFunction, PolynomialNoise,
//     Trained as HeteroscedasticGprTrained,
// };
...
pub struct LmcTrained {
    ...
    Y_train: Array2<f64>,
    kernels: Vec<Box<dyn Kernel>>,
    mixing_matrices: Vec<Array2<f64>>,
    alpha: f64,
    n_outputs: usize,
    n_latent: usize,
    gram_matrices: Vec<Array2<f64>>, // Gram matrices for each kernel
    alpha_vectors: Vec<Array1<f64>>, // Alpha vectors for each latent function
...
impl LinearModelCoregionalization<LmcTrained> {
    ...
    pub fn latent_contributions(&self, X: &ArrayView2<f64>) -> SklResult<Vec<Array2<f64>>> {
        ...
        for r in 0..self._state.n_latent {
            ...
            let K_star = self._state.kernels[r]
