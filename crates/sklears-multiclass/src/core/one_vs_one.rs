    ...
    T: Predict<Array2<Float>, Array1<Float>>,
{
    ...
    fn predict_rank_voting(&self, x: &Array2<Float>) -> SklResult<Array1<i32>> {
        ...
        for sample_idx in 0..n_samples {
            ...
            // Collect all pairwise predictions with confidence scores
            let mut pairwise_predictions = Vec::new();
                ...
                self.base_estimator.class_pairs.iter().enumerate()
            {
                if let Some(estimator) = self.base_estimator.estimators.get(pair_idx) {
                    ...
                    let (winner_idx, loser_idx) = if score > 0.5 {
                        (class_i_idx, class_j_idx)
                    } else {
                        (class_j_idx, class_i_idx)
                    };

                    pairwise_predictions.push((winner_idx, loser_idx, confidence));
                }
            }

            // Sort by confidence in descending order (most confident first)
            pairwise_predictions.sort_by(|a, b| b.2.partial_cmp(&a.2).expect("operation should succeed"));

            // Assign rank-based scores
            for (rank, &(winner_idx, _loser_idx, confidence)) in
                pairwise_predictions.iter().enumerate()
            {
                let rank_weight = (pairwise_predictions.len() - rank) as f64;
                let weighted_score = rank_weight * confidence;
                rank_scores[winner_idx] += weighted_score;
            }

            // Predict class with highest rank score
            let max_idx = rank_scores
                .iter()
                .enumerate()
                .max_by(|(_, a): &(_, &Float), (_, b)| (**a).partial_cmp(*b).expect("operation should succeed"))
                .map(|(idx, _)| idx)
    ...
    fn predict_distance_voting(&self, x: &Array2<Float>) -> SklResult<Array1<i32>> {
        ...
        for sample_idx in 0..n_samples {
            ...
            // Get weighted votes based on distance from decision boundary
            for (pair_idx, &(class_i, class_j)) in
                self.base_estimator.class_pairs.iter().enumerate()
            {
                if let Some(estimator) = self.base_estimator.estimators.get(pair_idx) {
                    ...
                    // Higher distance means more confident prediction
                    let distance = (score - 0.5).abs();

                    // Weight the vote by distance from boundary
                    if score > 0.5 {
                        // Vote for class_i with weight proportional to distance
                        distance_scores[class_i_idx] += distance;
                    } else {
                        // Vote for class_j with weight proportional to distance
                        distance_scores[class_j_idx] += distance;
                    }
                }
            }

            // Predict class with highest distance-weighted score
