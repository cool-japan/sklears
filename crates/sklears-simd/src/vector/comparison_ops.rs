//! # SIMD Vector Comparison and Logical Operations
//!
//! High-performance SIMD-optimized comparison and logical operations for vectors.
//! Provides element-wise comparison operations that return boolean masks and
//! logical operations for combining boolean vectors.
//!
//! ## Features
//!
//! - **Element-wise Comparisons**: Equal, not-equal, less-than, greater-than, etc.
//! - **Logical Operations**: AND, OR, XOR, NOT operations on boolean masks
//! - **Boolean Mask Support**: Efficient boolean vector operations
//! - **Multi-Platform SIMD**: SSE2, AVX2, AVX512, NEON optimizations
//! - **Automatic Fallback**: Graceful fallback to scalar implementations
//!
//! ## Performance Notes
//!
//! Comparison operations return boolean masks that can be efficiently processed
//! using SIMD instructions. The logical operations can be chained for complex
...
unsafe fn and_vec_sse2(a: &[bool], b: &[bool], result: &mut [bool]) {
    ...
    // Process 16 elements at a time (packed as bytes)
    while i + 16 <= a.len() {
        #[cfg(feature = "no-std")]
        use core::arch::x86_64::*;
        #[cfg(not(feature = "no-std"))]
...
unsafe fn and_vec_avx2(a: &[bool], b: &[bool], result: &mut [bool]) {
    ...
    // Process 32 elements at a time (packed as bytes)
    while i + 32 <= a.len() {
        #[cfg(feature = "no-std")]
        use core::arch::x86_64::*;
        #[cfg(not(feature = "no-std"))]
...
unsafe fn and_vec_avx512(a: &[bool], b: &[bool], result: &mut [bool]) {
    ...
    // Process 64 elements at a time (packed as bytes)
    while i + 64 <= a.len() {
        #[cfg(feature = "no-std")]
        use core::arch::x86_64::*;
        #[cfg(not(feature = "no-std"))]
...
unsafe fn ne_vec_neon(a: &[f32], b: &[f32], result: &mut [bool]) {
    ...
    while i + 4 <= a.len() {
        let a_vec = vld1q_f32(a.as_ptr().add(i));
        let b_vec = vld1q_f32(b.as_ptr().add(i));
        // Negate equality comparison for not-equal
...
mod tests {
    ...
    fn test_comparison_properties() {
        ...
        // Test that eq and ne are complementary
        for i in 0..4 {
        ...
        // Test that lt and ge are complementary
        for i in 0..4 {
    ...
    fn test_large_vectors() {
        // Test with vectors larger than SIMD register sizes
        let size = 100;
        ...
        // All elements should be true since a[i] < b[i] for all i
